...
def training(args, dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, wizard=None, progress_bar=None):
    if args.use_dust3r:
    ...
    if wizard is not None:
        set_progress_bar(wizard, progress_bar, opt.iterations - first_iter)
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        while network_gui.conn != None:
            try:
                ...
                if custom_cam != None:
                    ...
                    net_image_bytes = memoryview((torch.clamp(net_image, min=0, max=1.0)
        ...
        # Loss
        loss, Ll1 = cal_loss(opt, args, image, render_pkg, viewpoint_cam, bg, tb_writer=tb_writer,
                             iteration=iteration, mono_loss_type=args.mono_loss_type)
...
def cal_loss(opt, args, image, render_pkg, viewpoint_cam, bg, silhouette_loss_type="bce", mono_loss_type="mid", tb_writer: Optional[SummaryWriter] = None, iteration=0):
    ...
    if args.use_dust3r:
        ...
        tracking_loss = get_loss_tracking(
            image_ab, render_pkg["rendered_alpha"], viewpoint_cam) + args.lambda_t_norm * torch.abs(viewpoint_cam.cam_trans_delta).mean()

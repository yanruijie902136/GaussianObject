...
def training(args, dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, conn=None):
    if args.use_dust3r:
    ...
    if conn is not None:
        create_progress_bar(conn, opt.iterations - first_iter)
...
def main(argv=None, conn=None):
    ...
    training(args, lp.extract(args), op.extract(args), pp.extract(args), args.test_iterations,
             args.save_iterations, args.checkpoint_iterations, args.start_checkpoint, args.debug_from, conn)
